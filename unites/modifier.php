...
$stmt = $pdo->prepare('SELECT * FROM unites WHERE id_unite = ?');
$stmt->execute([$id]);
$unite = $stmt->fetch();

if (!$unite) {
    $_SESSION['flash_error'] = 'Unité introuvable.';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nomUnite === '' || $symbole === '') {
        ...
        $stmt = $pdo->prepare('UPDATE unites SET nom_unite = ?, symbole = ? WHERE id_unite = ?');
        $stmt->execute([$nomUnite, $symbole, $id]);

        $_SESSION['flash_success'] = 'Unité modifiée avec succès.';
        header('Location: /unites/index.php');
        exit;
    }
}
...
<head>
    ...
    <title>Modifier une unité - PharmaGest</title>
    ...
    <div class="app-layout">
        ...
        <div class="main-content">
            ...
            <div class="page-content">
                <h1 class="page-title">Modifier une unité</h1>
                    ...
                    <form method="POST">
                        <div class="form-group">
                            ...
                            <input type="text" id="nom_unite" name="nom_unite" required
                                   value="<?= htmlspecialchars($_POST['nom_unite'] ?? $unite['nom_unite']) ?>">
                        </div>
                        <div class="form-group">
                            <label for="symbole">Symbole *</label>
                            <input type="text" id="symbole" name="symbole" required
                                   value="<?= htmlspecialchars($_POST['symbole'] ?? $unite['symbole']) ?>">
                        </div>
