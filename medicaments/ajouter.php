...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($idCategorie === 0 || $reference === '' || $nom === '') {
        ...
        try {
            $stmt = $pdo->prepare(
                'INSERT INTO medicaments (id_categorie, reference, nom, dosage, forme, description, seuil_alerte)
                 VALUES (?, ?, ?, ?, ?, ?, ?)'
            );
            $stmt->execute([$idCategorie, $reference, $nom, $dosage ?: null, $forme ?: null, $description ?: null, $seuilAlerte]);

            $_SESSION['flash_success'] = 'Médicament ajouté avec succès.';
...
<head>
    ...
    <title>Ajouter un médicament - PharmaGest</title>
    ...
    <div class="app-layout">
        ...
        <div class="main-content">
            ...
            <div class="page-content">
                <h1 class="page-title">Ajouter un médicament</h1>
                ...
                <div class="card form-card">
                    ...
                    <?php if (empty($categories)): ?>
                        <p>⚠️ Aucune catégorie n'existe encore. <a href="/categories/ajouter.php">Créer une catégorie</a> avant d'ajouter un médicament.</p>
                    <?php else: ?>
                    <form method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="reference" name="reference" required placeholder="Ex. MED001"
                                   value="<?= htmlspecialchars($_POST['reference'] ?? '') ?>">
                        </div>
                        <div class="form-group">
                            <label for="nom">Nom *</label>
                            <input type="text" id="nom" name="nom" required placeholder="Ex. Paracétamol"
                                   value="<?= htmlspecialchars($_POST['nom'] ?? '') ?>">
                        </div>
                        <div class="form-group">
                            <label for="dosage">Dosage</label>
                            <input type="text" id="dosage" name="dosage" placeholder="Ex. 500 mg"
                                   value="<?= htmlspecialchars($_POST['dosage'] ?? '') ?>">
                        </div>
                        <div class="form-group">
                            <label for="forme">Forme</label>
                            <input type="text" id="forme" name="forme" placeholder="Ex. Comprimé"
                                   value="<?= htmlspecialchars($_POST['forme'] ?? '') ?>">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" id="seuil_alerte" name="seuil_alerte" min="0"
                                   value="<?= htmlspecialchars($_POST['seuil_alerte'] ?? '10') ?>">
                        </div>
